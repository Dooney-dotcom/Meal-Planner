...
export default function SortableFoodItem({ food, categories, setCategories, day, slot, updateFood, removeFood, copyFood }) {
  ...
  if (isEditing) {
    return (
      <li ref={setNodeRef} style={style} className="block list-none">
        <InlineFoodForm 
          initialData={food}
          onSave={(updatedData) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <button className="text-slate-300 hover:text-slate-500 cursor-grab active:cursor-grabbing p-1 -ml-1" {...attributes} {...listeners}>
      ...
      <div className="no-print flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => copyFood(food)} className="text-slate-400 hover:text-blue-600 p-2 rounded-full hover:bg-blue-50 transition-colors" title="Copy Food"><Copy className="w-4 h-4" /></button>
        <button onClick={() => setIsEditing(true)} className="text-slate-400 hover:text-[#005F6A] p-2 rounded-full hover:bg-[#005F6A]/5 transition-colors" title="Edit"><Edit2 className="w-4 h-4" /></button>
        <button onClick={() => removeFood(food.id)} className="text-slate-400 hover:text-red-600 p-2 rounded-full hover:bg-red-50 transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
      </div>
    </li>
  );
}
