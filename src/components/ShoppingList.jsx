...
function ShoppingList({ meals, categories }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 max-w-4xl mx-auto mb-8 print-avoid-break">
      <div className="flex justify-between items-center mb-8 pb-4 border-b border-slate-200">
        ...
        {checkedItems.size > 0 && (
          ...
          >
            <Trash2 className="w-4 h-4" /> Clear Checked ({checkedItems.size})
          </button>
        )}
      </div>
      ...
      {!hasItems ? (
        <div className="text-center py-16 bg-slate-50 rounded-lg border border-dashed border-slate-200">
          <p className="text-slate-600 text-lg font-medium">Your shopping list is empty.</p>
          <p className="text-slate-500 mt-1 text-sm">Add meals to your planner to generate the list.</p>
        </div>
      ) : (
          ...
          {groupedList.map(group => {
            ...
            return (
              <div key={group.category} className="print-avoid-break">
                <h3 className={`inline-block px-3 py-1 rounded text-xs font-bold tracking-wider uppercase mb-3 border ${getCategoryColor(categories, group.category)}`}>
                  {group.category}
                </h3>
